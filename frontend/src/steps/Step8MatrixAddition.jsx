...
export default function Step8MatrixAddition({ onSelectStep }) {
  ...
  return (
    ...
    >
      ...
      {/* FLOATING CURSOR TOOLTIP (Safe for desktop and mobile) */}
      {hoveredCell && hoveredCell.x !== undefined && (
        <div 
          className="cursor-tooltip"
          style={{
            position: 'fixed',
            left: `${Math.min(hoveredCell.x + 14, window.innerWidth - 220)}px`,
            top: `${Math.min(hoveredCell.y + 14, window.innerHeight - 60)}px`,
            pointerEvents: 'none',
            zIndex: 9999,
        ...
        >
          ...
          <span style={{ opacity: 0.3 }}>{operation === 'add' ? '+' : '−'}</span>
